...
sub _field ($$;$) {
	...
	if ( defined $self->{$field} ) {
		$Backticks::default_debug
			&& print STDERR "'$field' from self = '".$self->{$field}."'\n";
		return $self->{$field};
	}
	elsif ( defined $info{'package'} && $info{'package'} && defined ${'Backticks::'.$field}) {
		$Backticks::default_debug
			&& print STDERR "'$field' from package = '".${'Backticks::'.$field}."'\n";
		return ${'Backticks::'.$field};
	}
	elsif ( defined $info{'default'} ) {
		$Backticks::default_debug
			&& print STDERR "'$field' from default = '".$info{'default'}."'\n";
		return $info{'default'};
	}
	else {
		$Backticks::default_debug
...
sub run {
	...
	if ( $@ ) {
		...
		# If we have a non-zero exit code then the command went askew
	 	$self->_add_error("Failed with non-zero exit code ".$self->exitcode);
	}
...
# If called as a class method, will re-run the last backticks command
# If called as an instance method, will re-run that instance's command
...
# Reset this instance or the last run instance's state back as if the command
# had not been run
sub reset {
	my $self = _self(@_);
...
# Format the output of this instance or the last command run instance,
# as a table and return it as a string
sub as_table {
	my $self = _self(@_);
	my $p = sub {
		my $val = shift;
...
Boolean, returns whether or not $obj is configured to die when $obj->success is
not true.  Defaults to $Backticks::autodie or 0

=head2 $obj->chomped

Boolean, returns whether or not $obj is configured to chomp the contents of
stderr and stdout.  Defaults to $Backticks::chomped or 0

=head2 $obj->debug

Boolean, whether or not $obj is configured to print additional debugging output
to stderr as commands are processed.  Defaults to $Backticks::debug or 0
...
String, returns a description of any errors encountered while running the
command.

=head2 $obj->returncode
...
Boolean, whether or not a coredump was created when the command was run.
Equivalent to $? & 128

=head2 $obj->exitcode

Integer, the exit code returned when the command was run.  Equivalent to
$? >> 8

=head2 $obj->signal

Integer, the signal number returned when the command was run.  Equivalent to
$? & 127

=cut

# Get the specific fields from this instance or the last run instance
...
If the command run for $obj was not successful, this will contain a description
of the command which was run, the problem found, and the contents of the
command's STDERR.
